...
*/
function download(id, filename, styleSheets) {
    ...
    for (let i = 0; i < styleSheets.length; i += 1) {
        if (
          ...
          styleSheets[i].cssRules.length > 2
        ) {
          for (let j = 0; j < styleSheets[i].cssRules.length; i += 1) {
            if (
              styleSheets[i].cssRules[j].cssText
                .toLowerCase()
                .includes('hatch')
            ) {
